...
{
public:
    QAtChatPrivate( QSerialPort *_device )
    {
        fromChar = 'F';
        toChar = 'T';
        notifyChar = 'N';
...
QAtChat::QAtChat( QSerialPort *device )
    : QObject( device )
{
    ...
    d->matcher->add( "+CUSD:", QPrefixMatcher::Notification );
    d->matcher->add( "+CMGS:", QPrefixMatcher::Notification );
    d->matcher->add( "+CSQ:", QPrefixMatcher::Notification );
...
void QAtChat::abortDial()
{
    if ( d->first && d->first->d->command.startsWith( "ATD" ) ) {
        //FIXME : realize abort dial function
        //d->device->abortDial();
    ...
    switch ( d->matcher->lookup( line, command ) )
        {
            ...
            case QPrefixMatcher::Unknown:
            {
                ...
                if ( ! d->lastNotification.isEmpty() ) {
                    qDebug()<<"AtChat:" << d->notifyChar << ":" << line;
                ...
                if ( d->first ) {
                    qDebug()<<"AtChat:" << d->fromChar << ":" << line;
                    d->first->d->result.append( line );
                    return true;
                } else {
                    qDebug()<<"AtChat:" << d->unknownChar << ":" << line;
                }
            }
            break;
                    ...
                    if ( !d->wakeupInProgress )
                    ...
                    else
                        {
                            ...
                            if ( d->matcher->lookup( d->line, command ) == QPrefixMatcher::Notification )
                                qDebug()<<"AtChat:" << d->notifyChar << ":" << d->line;
                            else
                                qDebug()<<"AtChat:" << "W :" << d->line;
                        }
                    d->line = "";
                    posn = 0;
                }
...
{
    if ( !d->device->isOpen() && d->first ) {
        qDebug()<<"AtChat:" << d->toChar << ":"
...
void QAtChat::waitDataTimeout()
{
    if ( d->line.isEmpty() ) {
        primeIfNecessary();
    } else if ( d->line == d->tempLine ) {
        // Is garbage in input buffer?
        d->line = QString();
        primeIfNecessary();
    } else {
        // Give time to process the incoming data
        d->tempLine = d->line;
        QTimer::singleShot( 200, this, SLOT(waitDataTimeout()) );
    }
}
